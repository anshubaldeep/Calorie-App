...
export default function Home() {
  ...
  const [monthlyPriceThresholdReached, setMonthlyPriceThresholdReached] =
    useState(false)
  const initialData = {
    itemName: "",
    itemPrice: "",
  ...
  const getProducts = (date) => {
    ...
    const q = query(
      ...
      where("emailUsed", "==", sessionStorage.getItem("User")),
      where("dateString", "==", format(date, "MM/dd/yyyy")),
  ...
  const getProductsForMonth = (date) => {
    const q = query(
      collection(db, "FoodItems"),
      where("emailUsed", "==", sessionStorage.getItem("User")),
      where("monthString", "==", format(date, "MM")),
      orderBy("consumedAt", "asc")
    )
    onSnapshot(q, (querySnapshot) => {
  ...
  const getUserData = () => {
    const q = query(
      collection(db, "Users"),
      where("user", "==", sessionStorage.getItem("User"))
    )
    onSnapshot(q, (querySnapshot) => {
  ...
  useEffect(() => {
    ...
    if (val > threshold && threshold) {
      toast.error(
        `You exceeded your calorie threshold of ${threshold} calories on ${format(
          filterDate,
          "do LLL yyyy"
        )}`
      )
  ...
  useEffect(() => {
    let val = 0
    productsForMonth.length &&
      productsForMonth.forEach((item) => {
        val += item.itemPrice
      })
    if (val > moneyThreshold && moneyThreshold) {
      toast.error(
        `You exceeded your monthly money threshold of ${moneyThreshold} USD in ${format(
          filterDate,
          "LLL"
        )}`
      )
  ...
  return (
    <div className="homeSection">
      ...
      {thresholdReached && threshold && (
        <div className="thresholdReached">
          <Typography variant="h5">
            You exceeded your calorie threshold of {threshold} calories on{" "}
            {format(filterDate, "do LLL yyyy")}
          </Typography>
        </div>
      )}
      {monthlyPriceThresholdReached && moneyThreshold && (
        <div className="thresholdReached">
          <Typography variant="h5">
            You exceeded your monthly money threshold of {moneyThreshold} USD in{" "}
            {format(filterDate, "LLL")}
          </Typography>
        </div>
      )}
      ...
      {loading ? (
        ...
        <Table rows={products} rowTitles={rowTitles} />
      ) : (
        <h1>No Items Covered that day!</h1>
      )}
